...
export default class PressToStart {
  ...
  constructor(canvas: HTMLCanvasElement, textImgPos: Position, dimensions: Dimensions) {
    ...
    // Restrict maximum width and height
    const MAX_WIDTH = 600;
    const MAX_HEIGHT = 150;
    this.canvasDimensions.WIDTH = Math.min(dimensions.WIDTH, MAX_WIDTH);
    this.canvasDimensions.HEIGHT = Math.min(dimensions.HEIGHT, MAX_HEIGHT);
  ...
  update(deltaTime: number, fadeOut = false) {
    ...
    this.drawEmoji("Want to play a game while you are here ?", fadeOut)
