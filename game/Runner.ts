...
export default class Runner {
  ...
  playIntro() {
    if (!this.activated && !this.crashed) {
      ...
      // Limit the container width to the maximum allowed width
      const targetWidth = Math.min(this.dimensions.WIDTH, MAX_WIDTH);
  
      // let keyframes = `@-webkit-keyframes intro {
      //     from { width: ${Trex.config.WIDTH}px }
      //     to { width: ${targetWidth}px }
      //   }`
      let keyframes = `@-webkit-keyframes intro {}`;
  ...
  setArcadeModeContainerScale() {
    ...
    if (IS_MOBILE) {
      ...
      // On PC screens, do not change the scale
      this.containerEl.style.transform = "none"; // Reset transform to avoid scaling
    }
  }
  ...
  static updateCanvasScaling(canvas: HTMLCanvasElement, opt_width?: number, opt_height?: number) {
    ...
    // Calculate desired width and height based on max limits
    const targetWidth = Math.min(opt_width || canvas.width, MAX_WIDTH);
    const targetHeight = Math.min(opt_height || canvas.height, MAX_HEIGHT);
    ...
    if (devicePixelRatio !== backingStoreRatio) {
      ...
      // Scale the context to counter the fact that we've manually scaled the canvas element
      context.scale(ratio, ratio);
      return true;
    } else if (devicePixelRatio === 1) {
      // Reset the canvas width / height when zooming or switching screens with devicePixelRatio change
      canvas.style.width = `${targetWidth}px`;
      canvas.style.height = `${targetHeight}px`;
    }
    return false;
